...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        # 1) merged_masks_meta.json 을 읽어 딕셔너리로 저장
        meta_path = Path(self.mask_dir) / "merged_masks_meta.json"
        with open(meta_path, 'r') as f:
            meta_list = json.load(f)
        # filename → 메타 엔트리
        self.merged_meta_dict = { e["filename"]: e for e in meta_list }
    ...
    # for robust segmentation information clustur
    # revised by hochan

    
    def semantic_robust_cluster_mask(self,
                                     inlier_sf: torch.Tensor,
                                     features: torch.Tensor,
                                     base: str,
                                     K_target: int = 80) -> torch.Tensor:
        """
        Semantic 초기 클러스터 + agglomerative merging with semantic penalty
        inlier_sf:   [1,1,Hf,Wf] robust inlier mask
        features:    [C_feat, Hf, Wf] feature map
        base:        frame basename for .npz lookup
        K_target:    목표 클러스터 개수
        """
        device = self.device
        # 1) output 해상도
        _, _, Hf, Wf = inlier_sf.shape

        # 2) .npz 로딩 → segs, classes
        if not hasattr(self, "_npz_cache"):
            self._npz_cache = {}
        entry   = self.merged_meta_dict.get(base, {})
        npz_fp  = os.path.join(self.cfg.mask_dir, entry["segment_npz"])
        if npz_fp not in self._npz_cache:
            self._npz_cache[npz_fp] = np.load(npz_fp)
        npz_dat = self._npz_cache[npz_fp]
        segs, classes = npz_dat["masks"], npz_dat["classes"]

        # 3) semantic_masks dict 생성 (BoolTensor [Hf,Wf])
        semantic_masks: dict[int, torch.BoolTensor] = {}
        for seg_np, cls in zip(segs, classes):
            seg_t = torch.from_numpy(seg_np).float().unsqueeze(0).unsqueeze(0).to(device)  # [1,1,H,W]
            seg_rs = F.interpolate(seg_t, size=(Hf, Wf), mode="nearest")
            mask_bool = seg_rs.squeeze().bool()
            cid = int(cls)
            semantic_masks.setdefault(cid, mask_bool.clone()).logical_or_(mask_bool)

        # 4) cluster initialization
        clusters: list[dict] = []
        for cid, mask in semantic_masks.items():
            coords = torch.nonzero(mask, as_tuple=False).cpu().numpy()
            clusters.append({"pixels": coords, "label": cid})

        combined = torch.zeros((Hf, Wf), dtype=torch.bool, device=self.device)
        for m in semantic_masks.values():
            combined |= m
        ys, xs = torch.nonzero(~combined, as_tuple=True)
        # 5) merge cost with semantic penalty
        alpha = getattr(self.cfg, "cluster_alpha", 0.8) # semantic penalty weight
      
        def merge_cost(a, b): #if this return is small, merged first
        # variance increase
            fa = features[:, a["pixels"][:,0], a["pixels"][:,1]]
            fb = features[:, b["pixels"][:,0], b["pixels"][:,1]]
            var0 = fa.var(dim=1, correction=0).sum() + fb.var(dim=1, correction=0).sum() # var before merge
            fab = torch.cat([fa, fb], dim=1)
            var1 = fab.var(dim=1, correction=0).sum() # var after merge
            dv = (var1 - var0).item()
            # semantic penalty
            la, lb = a["label"], b["label"]
            sem_pen = 0.0 if (la is not None and la == lb) else alpha
            return dv + sem_pen   
        #-------------------------------------------------------------------
        # 6) build priority queue  -> time complexity O(N^2 log N)
        heap = []
        N = len(clusters)
        for i in range(N):
            for j in range(i+1, N):
                cost = merge_cost(clusters[i], clusters[j])
                heapq.heappush(heap, (cost, i, j))

        # 7) agglomerative merging
        while len([c for c in clusters if c is not None]) > K_target and heap:
            cost, i, j = heapq.heappop(heap)
            if clusters[i] is None or clusters[j] is None:
                continue
            # merge j into i
            new_pixels = np.vstack([clusters[i]["pixels"], clusters[j]["pixels"]])
            li, lj = clusters[i]["label"], clusters[j]["label"]
            new_label = li if li == lj else None
            clusters[i] = {"pixels": new_pixels, "label": new_label}
            clusters[j] = None
            # recompute costs for i vs others
            for k, cl in enumerate(clusters):
                if cl is None or k == i: continue
                heapq.heappush(heap, (merge_cost(clusters[i], cl), i, k))

        # 8) build final mask
        final = torch.zeros((Hf, Wf), device=self.device, dtype=torch.float)
        for c in clusters:
            if c is None: continue
            ys, xs = c["pixels"].T
            final[ys, xs] = 1.0

        # 9) combine with inlier_sf
        out = torch.logical_and(final.bool(),
                                inlier_sf.squeeze(1).squeeze(0)).float()
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            # img_id = int(image_ids.cpu().item())
            # imageio.imwrite(os.path.join(gt_train_dir,  f"{img_id}.png"),  gt_np)
            # imageio.imwrite(os.path.join(rend_train_dir, f"{img_id}.png"),  rend_np)
            

            # ─── 원본 파일명으로 저장하기 ─────────────────────────
            # img_id = int(image_ids.cpu().item())
            # # parser.image_names 에는 원본 파일명(예: "00012.jpg")이 들어 있음
            # orig = self.parser.image_names[img_id]
            # base = Path(orig).stem       # "00012"
            # name = base + ".png "        # "00012.png"
            ...
            # print("[DEBUG] TRAIN gt and rendered images are written")

            # # ─── optional: mask_dir 에서 동일 이름의 마스크를 복사하고 싶다면 ──
            # if self.mask_dir:
            #     msrc = os.path.join(self.mask_dir, name)
            #     mdst = os.path.join(cfg.result_dir, "train", "mask", name)
            #     os.makedirs(os.path.dirname(mdst), exist_ok=True)
            #     if os.path.isfile(msrc):
            #         shutil.copy(msrc, mdst)
            #         print("[DEBUG] mask images are copied")
            ...
            #     # 2) 없으면 하위 폴더까지 재귀 검색
            #     if not mpath.exists():
            #         candidates = list(src_root.rglob(f"{Path(name).stem}.*"))
            #         mpath = candidates[0] if candidates else None
            ...
            #         print(f"[DEBUG] mask copied: {mpath} → {mdst}")
            #     else:
            #         print(f"[WARN] mask not found for '{name}' under '{src_root}'")


            if self.mask_dir:
                src_root = Path(self.mask_dir)
                # 1) 최상위 폴더에서 확장자 무관하게 파일 찾기
                candidates = list(src_root.glob(f"{Path(name).stem}.*"))
                if not candidates:
                    # 2) 없으면 하위 폴더까지 재귀 검색
                    candidates = list(src_root.rglob(f"{Path(name).stem}.*"))
                mpath = candidates[0] if candidates else None
                ...
                if mpath:
                    ...
                    print(f"[WARN] mask not found for '{Path(name).stem}' under '{src_root}'")
            ...
            if cfg.loss_type == "l1":
                ...
                if cfg.semantics:
                    ...
                    if cfg.cluster:
                        ...
                        base = os.path.splitext(os.path.basename(data["filename"][0]))[0]
                        # 의미‧클러스터 보팅 + inlier-비율 필터
                        pred_mask_cf = pred_mask.permute(0,3,1,2)
                        pred_mask = self.semantic_robust_cluster_mask(
                            inlier_sf=pred_mask_cf,          # robust mask (1=신뢰 픽셀)
                            features=sf,                  # upsampled semantic feature map
                            base=base,                    # .npz 메타 조회용
                            K_target=getattr(cfg, "K_target", 80)  # 클러스터 수(옵션)
                        )
                        pred_mask = pred_mask.permute(0, 2, 3, 1)  # [1, H, W, 1]
            ...
            # in here, we use the gradients to update the running stats-> 4.1.2 mlp using mask updates
            # revised by hochan
            if self.mlp_spotless:
                # spotless 모듈 학습 모드 & spot loss 계산
                self.spotless_module.train()
                flat_pred = pred_mask_up.flatten()  # (H*W,)
                ...
                #.npz에서 per-frame 세그먼트 불러와 클래스별 OR 누적
                base   = os.path.splitext(os.path.basename(data["filename"][0]))[0]
                entry  = self.merged_meta_dict.get(base, {})
                npz_fp = os.path.join(self.cfg.mask_dir, entry["segment_npz"])  # segment_npz 필드
                npz_data = np.load(npz_fp)
                segs     = npz_data["masks"]    # shape (N_objects, H, W), uint8
                classes  = npz_data["classes"]  # shape (N_objects,), int

                # 예측 이진 마스크 (bool 텐서)
                pred_bool_flat = (flat_pred > 0.5)
                _, H_pred, W_pred, _ = lower_mask.shape

                #세그먼트별로 IoU 계산 → threshold 이상만 OR 누적
                min_iou = getattr(self.cfg, "min_iou_seg", 0.7) # IoU threshold
                class_masks = {}  # class_id → BoolTensor[H,W]

                for seg_np, cls in zip(segs, classes):
                    # (H,W) BoolTensor으로 변환
                   # a) to tensor & add batch+channel dims
                    seg_t = torch.from_numpy(seg_np.astype(np.float32)).unsqueeze(0).unsqueeze(0).to(self.device)
                    # b) resize to model output resolution
                    seg_rs = F.interpolate(seg_t, size=(H_pred, W_pred), mode="nearest")
                    seg_bool = seg_rs.squeeze(0).squeeze(0) > 0  # (H, W) bool
                    # c) flatten
                    seg_flat = seg_bool.flatten()

                    # Intersection & Union
                    inter = torch.logical_and(pred_bool_flat, seg_flat).sum()
                    union = torch.logical_or(pred_bool_flat, seg_flat).sum()
                    iou   = inter.float() / (union.float() + 1e-6)

                    # IoU 기준 통과 시에만 해당 세그먼트 누적
                    if iou >= min_iou:
                        class_masks.setdefault(
                            int(cls),
                            torch.zeros((H_pred * W_pred,), dtype=torch.bool, device=self.device)
                        ).logical_or_(seg_flat)

                # ────────────────────────────────────────────────────────────────
                # class_masks 기반 supervised loss 계산
                lambda_sup  = getattr(self.cfg, "lambda_sup", 1.0)

                pred_flat = flat_pred  # 확률
                sup_loss = 0.0
                n_sup    = 0

                for cmask_flat in class_masks.values():
                    target = cmask_flat.float()  # (H*W,)
                    sup_loss += F.binary_cross_entropy(flat_pred, target)
                    n_sup    += 1

                supervised_loss = sup_loss / n_sup if n_sup > 0 else torch.tensor(0.0, device=self.device)
                # ────────────────────────────────────────────────────────────────
                # regularizer & backward
                reg = 0.5 * self.spotless_module.get_regularizer()
                total_loss = spot_loss + lambda_sup * supervised_loss + reg
                total_loss.backward()
               

                # 5) 로그 출력
                # print(
                #     f"SpotLoss: {spot_loss.item():.4f}, "
                #     f"SupLoss: {supervised_loss.item():.4f}, "
                #     f"Reg: {reg.item():.4f}"
                # )
    ...
    def eval(self, step: int):
        ...
        for i, data in enumerate(valloader):
            ...
            # idx_str = f"{i:04d}"
            # imageio.imwrite(os.path.join(gt_test_dir ,  f"{idx_str}.png"),  gt_np)
            # imageio.imwrite(os.path.join(rend_test_dir, f"{idx_str}.png"),  rend_np)
            
            # ─── 원본 파일명으로 저장하기 ─────────────────────────
            # img_id = int(data["image_id"].cpu().item())
            # orig = self.parser.image_names[img_id]
            # print(self.parser.image_names)
            # base = Path(orig).stem
            
            # name = base + ".png"
            # base = Path(data["filename"]).stem
            # name = base.replace("_extra", "").replace("_clutter", "") + ".png"
            ...
            # if self.mask_dir:
            #     msrc = os.path.join(self.mask_dir, name)
            #     mdst = os.path.join(cfg.result_dir, "test_training", "mask", name)
            #     os.makedirs(os.path.dirname(mdst), exist_ok=True)
            #     if os.path.isfile(msrc):
            #         shutil.copy(msrc, mdst)
            #         print("[DEBUG] masks are copied")
            ...
            #     src_root = Path(self.mask_dir)
            #     # 1) 우선 최상위에서 찾기
            #     mpath = src_root / (Path(name).stem + ".jpg")
            #     # 2) 없으면 하위 폴더까지 재귀 검색
            #     if not mpath.exists():
            #         candidates = list(src_root.rglob(f"{Path(name).stem}.jpg"))
            #         mpath = candidates[0] if candidates else None

            #     mdst_dir = Path(self.cfg.result_dir) / "test" / "mask"
            #     mdst_dir.mkdir(parents=True, exist_ok=True)
            #     mdst = mdst_dir / (Path(name).stem + ".jpg")  # 저장 확장자 강제 .jpg

            #     if mpath and mpath.exists():
            #         shutil.copy(mpath, mdst)
            #         print(f"[DEBUG] mask copied: {mpath} → {mdst}")
            #     else:
            #         print(f"[WARN] mask not found for '{Path(name).stem}.jpg' under '{src_root}'")
        ...
        print("")
        print("[EVAL] test rendered image save and test metrics")
        print(
        ...
        print("")
        print("[EVAL] aving training rendered images in eval mode...")
        cfg = self.cfg
        device = self.device
