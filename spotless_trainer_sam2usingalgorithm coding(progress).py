...
class Config:
    ...
    bin_size: int = 10000 #in paper, there suggests 0.001 but it's not good.
    ...
    #model_base mask directory
    model_mask : bool = False
    #residual directory for check 3d grad
    residual : bool = False
    #top k value to save max residual pixels
    topk : int = 10
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        # revised
        # use error_points to put sam2 -> possible mask set
        self.error_points = {
             "best_point": {"h": -1, "w": -1, "z": -math.inf, "step": -1, "imname": None}, # [h, w, z], z is residual error
            "point_pq" : [], # heap to save high-residual point  
            "step": 0, 
            "imname": None        
    ...
    # revised-0901
    # make max-heap with error point update
    def update_error_points(self, error_per_pixel, step, imname=None, topk=None):
        """
        error_per_pixel: Tensor [1,H,W,3]
        step: int (global step)
        imname: str or int (optional)
        topk: save top k value
        """

        residual_map = error_per_pixel.mean(dim=-1).squeeze(0)  # [1,H,W,1] -> [H,W]
        H, W = residual_map.shape
        flat = residual_map.view(-1)                            # [H*W]

        
        vals, idxs = torch.topk(flat, k=topk)                   # vals: [topk], idxs: [topk]

        for v, idx in zip(vals.tolist(), idxs.tolist()):
            h = idx // W
            w = idx % W
            z = float(v)
            entry = (-z, h, w, int(step), imname)               # −z push → max-heap
            heapq.heappush(self.error_points["point_pq"], entry)

            # update best top k error point
            if z > self.error_points["best_point"]["z"]:
                self.error_points["best_point"] = {
                    "h": h, "w": w, "z": z,
                    "step": int(step), "imname": imname
                }

        # stable topk value remaning in heapq
        if topk is not None and len(self.error_points["point_pq"]) > topk:
            heapq.heappop(self.error_points["point_pq"])
    ...
    def train(self):
        ...
        trainloader_iter = iter(trainloader) # for training dataset size ex0 108 -> 94

        # Training loop.
        global_tic = time.time()
        pbar = tqdm.tqdm(range(init_step, max_steps)) # max_steps = 30000. thus all step with training will finish in 30000
                                                      # running & loss updating algorithm is in one step
                                                      # thus when we use all dataset-> 1 epoch but we fixed 30000step, thus it can be 25 epoch + 14 step = 30000        
        ...
        for step in pbar:
            ...
            except StopIteration: # when use all loaded train dataset, newly start the new dataset
            ...
            if cfg.loss_type == "l1":
                ...
                #revised
                #save error_per_pixel for 3d grad visualization.
                # if (step+1) % 200 ==0:
                #     if cfg.residual:
                #         error_per_pixel_dir = os.path.join(cfg.result_dir,"error_per_pixel")
                #         os.makedirs(error_per_pixel_dir, exist_ok=True)
                        
                #         residual_map = error_per_pixel.mean(dim=-1).squeeze(0)   # [H, W]
                #         residual_np = residual_map.detach().cpu().numpy() 
                #         np.save(os.path.join(error_per_pixel_dir, f"{step}.npy"), residual_np)
                #         print("-----saved error_per_pixel!")
                if cfg.residual:
                    self.update_error_points(error_per_pixel,step, imname, cfg.topk)
                    print("save topk error points in each step")
            ...
            if step > max_steps - 200 and cfg.semantics:
                ...
                #save only model_predicted mask
                # revised
                if cfg.model_mask:
                    predicted_mask = (log_pred_mask > 0.5).squeeze().cpu().numpy().astype(np.uint8)
                    predicted_mask = predicted_mask * 255
                    predicted_mask_dir= os.path.join(cfg.result_dir,"mask")
                    os.makedirs(predicted_mask_dir, exist_ok=True)
                    predicted_mask_path = os.path.join(predicted_mask_dir, f"mask_{imname}.png")
                    imageio.imwrite(predicted_mask_path,predicted_mask)
                    print("-----mask is saved!")
    ...
    def update_running_stats(self, info: Dict):
        ...
        # avg_err is final cut off value of mask.
    ...
    def eval(self, step: int):
        ...
        #batch size is 1 that means for one iter with loss updated, there uses one image and for one epoch, there uses whole images.
